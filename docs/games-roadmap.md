...
3. **Phase 2 — Platform foundations**: `apps/hub`, `packages/game-manifest`, the multi-site → single-site path-based routing/compose step, and the wizard's first version (scaffold-a-game-from-template, starting with Tetris as the one template that exists). This is also the natural point to start the React editor migration, since the wizard is the first genuinely React-shaped UI.
